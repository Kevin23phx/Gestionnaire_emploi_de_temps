...
| 14 | FR-REF-06 | ...DOIT permettre à un Enseignant de n'accéder qu'aux UFR auxquelles il est explicitement rattaché. | Le référentiel des enseignants porte la relation many-to-many Enseignant↔UFR ; la résolution de portée s'appuie sur cette table, jamais sur une déduction depuis les créneaux. | `ReferentielModule` — table de liaison `EnseignantUfr` ; RBAC Guard vérifie l'appartenance avant de servir le planning d'un enseignant pour une UFR donnée. |
| 15 | INT-08 / RM-06 | ...NE DOIT PAS permettre à un Étudiant d'être rattaché à deux UFR à la fois ; seul l'Admin peut transférer un Étudiant d'une UFR à une autre. | Le Service Référentiel Étudiants refuse tout changement d'UFR d'un Étudiant déjà rattaché, sauf appelant de rôle Admin. | `ReferentielModule` (`EtudiantsService`) — vérifie `role === "admin"` avant d'autoriser une modification du champ UFR d'un Étudiant existant. |
| 16 | FR-REF-08/09 | ...NE DOIT PAS créer un second Étudiant pour un INE déjà présent, quelle que soit l'UFR/l'année visée ; DOIT conserver l'année académique d'inscription. | Le Service d'Import applique la contrainte d'unicité INE existante et persiste l'année académique fournie. | `ReferentielModule` (`EtudiantsService.import`) — contrainte `@unique` déjà en place sur `Etudiant.ine` ; nouvelle colonne `anneeAcademique`. |
| 17 | FR-REF-10 | ...DOIT fournir un fichier modèle téléchargeable pour l'import d'étudiants. | Le Service Référentiel expose le canevas comme une ressource statique versionnée avec le format d'import attendu. | `ReferentielModule` — endpoint `GET /etudiants/canevas` (ou fichier statique servi par la PWA), colonnes alignées sur `EtudiantImportLigneDto`. |
| 18 | FR-REF-11 | ...DOIT permettre de filtrer les étudiants par année académique et par filière ("département"). | Le Service Référentiel expose ces deux champs comme paramètres de requête indexés. | `ReferentielModule` (`EtudiantsService.list`) — filtres `anneeAcademique`/`filiere` sur `GET /etudiants`, index composé en base. |
...
5. **Composants émergents** : les modules listés ci-dessus (Auth, Accounts, `UfrModule` [V2, nouveau], Referentiel, Planning, ConflictEngine, Audit, Notification, Request, Sync) forment la liste d'entrée du C4 Composant (étape 5).
