...
> **`[V8]` Révision du 2026-09-21 — « Parcours » se scinde en Niveau et Spécialité, et l'espace Admin est dissimulé.** Deux demandes du chef de projet, sans rapport entre elles.
>
> (1) Le champ intitulé « Parcours » à l'écran contenait des **niveaux** du cycle LMD (L1…M2) — un mot pour deux notions. Les deux sont séparés : le **niveau** reste la liste close du LMD, la **spécialité** devient une entrée de référentiel que le Gestionnaire déclare et rattache à un **couple (département, niveau)**, ce qui permet à une licence de portail (MPCI) de n'offrir aucun choix en L1 et quatre en L2. Deux invariants s'ajoutent : **INV-20** et **INV-21**. Aucun invariant n'est affaibli — en particulier **INT-07 reste entier** : une spécialité n'a pas d'UFR propre, elle en hérite par son département, et c'est par lui que le cloisonnement s'applique.
>
> (2) L'espace Admin était servi sous `/admin` et **redirigeait** vers la page de connexion tout visiteur non autorisé, lui confirmant ainsi l'existence de la section. Il est déplacé sous une adresse non annoncée, réglable par configuration, et répond désormais **404** comme n'importe quelle adresse inexistante. **INT-12** consigne ce que cette mesure ne vaut PAS : ce n'est pas un contrôle d'accès, et rien de ce qui suit ne doit jamais s'y fier — INV-11 et INT-07 restent les seules garanties réelles.

> **`[V8.1]` Révision du 2026-09-21 (même journée) — l'affectation se porte sur le CRÉNEAU.** Retour du porteur de projet dès la mise en main de la V8 : porter la spécialité sur le Groupe obligeait à dupliquer les promotions (quatre « L2 Médecine ») et donc à ressaisir tout le tronc commun autant de fois. C'est désormais le créneau qui porte l'affectation ; un créneau sans affectation concerne toute la promotion. **INV-22** s'ajoute, et **INV-02 n'est PAS touché** : le verrou physique anti-double-réservation porte sur la salle, pas sur le groupe. La seule règle qui s'assouplit est celle du conflit de GROUPE (02_SRS FR-CONF-03), et elle le fait sur un fait objectif — deux spécialités différentes désignent deux sous-populations disjointes.

> **`[V7]` Révision du 2026-09-16 — exception ciblée à INV-11 et à INT-07.** Retour des gestionnaires lors de la présentation, deux exceptions ponctuelles, chacune limitée à un seul référentiel : (1) l'Admin doit pouvoir créer un département si le Gestionnaire de l'UFR concernée n'est pas disponible (`POST /api/departements` accepte désormais le rôle `admin`, à condition de fournir explicitement l'`ufrId` — jamais déduit de la session, l'Admin n'en ayant aucune) ; (2) les salles cessent d'être rattachées à une UFR — un Gestionnaire dont les salles sont toutes occupées doit pouvoir en trouver une ailleurs, donc le référentiel des salles devient unique et partagé par toute l'université (seule l'unicité du nom reste garantie). Aucune des deux n'est un assouplissement général : le reste du référentiel (cours, groupes), le planning, les conflits et l'audit restent entièrement cloisonnés par UFR comme avant.
...
|---|---|
| **Visiteur public** `[V3]` | Retrouver le programme de n'importe quel groupe de n'importe quelle UFR, sans compte, par la cascade UFR → département → niveau → groupe ; le conserver en favori sur son appareil ; l'abonner à son agenda personnel et en recevoir automatiquement les évolutions ; l'activer en alerte ; le consulter hors-ligne. |
...
| INV-17 `[V3.2]` | Deux départements d'un même établissement ne portent jamais le même libellé, à la casse près. C'est la garantie qui empêche « Informatique » et « informatique » d'apparaître comme deux départements distincts dans la cascade publique (FR-PUB-02). Le même libellé dans deux établissements différents reste légitime — « Informatique » existe réellement à l'UFR/SEA et à l'IBAM. |
| INV-20 `[V8]` | Deux spécialités ne portent jamais le même libellé, à la casse près, **pour un même couple (département, niveau)**. Même garantie qu'INV-17 pour les départements, et pour la même raison : c'est ce qui empêche « Informatique » et « informatique » d'apparaître comme deux choix distincts dans la cascade publique (FR-PUB-02). Le même libellé à **deux niveaux différents** du même département reste légitime — « Informatique » en L2 et en L3 sont deux choix offerts à deux cohortes distinctes, pas un doublon. |
| INV-21 `[V8]` | La spécialité portée par un groupe ne change jamais du fait d'une modification du référentiel des spécialités : renommer ou supprimer une spécialité ne réécrit aucun groupe existant. Même garantie que pour le département d'un groupe (`Groupe.departement`, `[V5]`) et pour la même raison : un emploi du temps déjà publié ne doit pas changer de nom rétroactivement. La contrepartie assumée est qu'un groupe peut porter une spécialité qui n'existe plus au référentiel — la cascade publique la propose alors quand même, faute de quoi le programme deviendrait introuvable. |
| INV-22 `[V8.1]` | Deux créneaux d'un même groupe qui se chevauchent ne sont tolérés QUE s'ils sont affectés à des spécialités différentes et non vides. Un créneau sans affectation concerne toute la promotion : il ne peut donc jamais chevaucher un autre créneau du même groupe, spécialisé ou non. Autrement dit, aucun étudiant ne peut se voir attribuer deux cours simultanés — c'est la garantie qu'INV-02 apportait déjà pour les salles, portée ici sur les personnes, et c'est elle que la règle du moteur de conflits (02_SRS FR-CONF-03) traduit. Elle reste franchissable par dérogation motivée (FR-CONF-07), comme tous les conflits bloquants. |
...
| INV-11 `[V2, exception V7]` | Un compte Admin n'a jamais de droit d'écriture sur le référentiel ou le planning d'une UFR quelconque — uniquement sur la création d'UFR/comptes Gestionnaire et le transfert d'UFR d'un Étudiant, **et, depuis `[V7]`, la création d'un département** (`ufrId` fourni explicitement dans la requête, jamais déduit). |
...
| INT-12 `[V8]` | L'emplacement de l'espace d'administration ne doit jamais être traité comme un secret sur lequel repose une garantie. Il n'est ni annoncé ni devinable, et l'adresse historique `/admin` répond comme une adresse inexistante — mais **aucune de ces mesures n'est un contrôle d'accès**, et aucune règle de ce contrat ne doit s'y adosser. Un chemin finit toujours par fuiter (journal de serveur, historique de navigateur, capture d'écran, script servi au navigateur — c'est le premier endroit que fouillent les guides de test d'intrusion, OWASP WSTG-CONF-05). La seule chose qui refuse réellement la donnée à qui connaît l'adresse est le contrôle de rôle côté serveur : INV-11, INT-07 et le RBAC de l'API. La dissimulation retire la cible des balayages automatisés, rien de plus, et elle est consignée ici précisément pour qu'on ne lui prête jamais davantage. |
| INT-12 étendue `[V8.2]` | Ce qui vaut pour l'emplacement de l'espace Admin vaut pour le geste d'accès à la connexion : le **double-clic sur le logo** (V8.2) ne doit jamais être traité comme un secret sur lequel repose une garantie. `/connexion` reste une adresse publique, joignable directement, et le fait qu'aucun bouton ne l'annonce ne protège rien par lui-même — cela retire seulement une cible de la page la plus consultée. Toute règle de ce contrat continue de s'adosser au contrôle de rôle côté serveur, jamais à la discrétion d'un geste d'interface. |
| INT-07 `[V2, exception V7]` | Un Gestionnaire de scolarité ne peut jamais lire ou écrire le référentiel, le planning, les conflits ou l'audit d'une UFR autre que la sienne, **à l'exception, depuis `[V7]`, du référentiel des salles** : les salles ne sont plus rattachées à une UFR (retour des gestionnaires — une salle d'une autre UFR doit rester trouvable quand celles de la sienne sont toutes occupées), donc lisibles et créables par n'importe quel Gestionnaire ou l'Admin. Le planning, les conflits et l'audit restent entièrement cloisonnés par UFR (dérivés du groupe, jamais de la salle). |
